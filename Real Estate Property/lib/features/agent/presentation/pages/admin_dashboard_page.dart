...
class _OverviewTab extends StatelessWidget {
            ...
            builder: (context, propSnapshot) {
              return StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, activeAgentSnapshot) {
                  return StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance.collection('bookings').where('status', isEqualTo: 'paid').snapshots(),
                    builder: (context, bookingsSnapshot) {
                      ...
                      if (bookingsSnapshot.hasData) {
                        for (var doc in bookingsSnapshot.data!.docs) {
                          ...
                          totalAdminRevenue += amount * 0.05; // 5% for admin
                        }
                      }
                      return SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _statCardFullWidth('Total Admin Revenue (5%)', '₱${totalAdminRevenue.toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => "${m[1]},")}', Icons.monetization_on_outlined, Colors.green.shade600),
  ...
  Widget _statCardFullWidth(String label, String value, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(label, style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.grey.shade600)),
                const SizedBox(height: 4),
                Text(value, style: GoogleFonts.inter(fontSize: 28, fontWeight: FontWeight.w800, color: const Color(0xFF0A1D37))),
              ],
            ),
          ),
        ],
...
class _ApplicationsTab extends StatelessWidget {
  ...
  Future<void> _approveAgent(BuildContext context, String userDocId) async {
    ...
    await NotificationService.sendNotification(
      ...
      body: 'Congratulations! Your agent application has been approved by the administrator.',
      type: 'agent_approval',
      referenceId: userDocId,
      referenceType: 'agent',
    );
  ...
  Future<void> _rejectAgent(BuildContext context, String userDocId) async {
    ...
    await NotificationService.sendNotification(
      ...
      body: 'We regret to inform you that your agent application was not approved at this time.',
                                    ...
                                    Wrap(
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: p.status == PropertyStatus.pending ? Colors.orange.shade50 : Colors.green.shade50,
                                            borderRadius: BorderRadius.circular(6),
                                            border: Border.all(color: p.status == PropertyStatus.pending ? Colors.orange.shade200 : Colors.green.shade200),
                                          ),
                                          child: Text(
                                            ...
                                            style: GoogleFonts.inter(
                                              fontSize: 9,
                                              fontWeight: FontWeight.w700,
                                              color: p.status == PropertyStatus.pending ? Colors.orange.shade700 : Colors.green.shade700,
                                            ),
                                          ),
                                        ),
                                      ],
                                        ...
                                        ElevatedButton(
                                          onPressed: () async {
                                            ...
                                            await NotificationService.sendNotification(
                                              ...
                                              body: 'Your property listing "${p.title}" has been approved by the administrator and is now live.',
                                              type: 'listing_status',
                                              referenceId: p.id,
